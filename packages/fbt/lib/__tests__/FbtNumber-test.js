...
describe('FbtNumber consistency', function () {
  FbtNumberConsistency.dataModuleNames.forEach(function (dataModuleName) {
    ...
    it('should be consistent with PHP in ' + dataModuleName, function () {
      for (var ii = 0; ii < phpNumberTypes.length; ++ii) {
        var jsType = jsModule.getVariation(ii); // In JS, we still have a table access (although there is only
        // one entry)

        jsType = jsType === '*' ? null : jsType;
        expect(jsType).toBe(phpNumberTypes[ii], ii + ' failed in ' + dataModuleName);
      }
    });
  });
});