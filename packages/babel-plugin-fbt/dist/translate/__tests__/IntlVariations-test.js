  ...
  describe('when compared with the client-side IntlVariations', () => {
    ...
    it('Number config must match client-side equivalent ', () => {
      ...
      for (const k in IntlNumberVariations) {
        expect(ClientSideIntlVariations).toHaveProperty(prefix + k, IntlNumberVariations[k]);
      }

      getPropsByPrefix(ClientSideIntlVariations, prefix).forEach(key => expect(IntlNumberVariations).toHaveProperty(key.substr(prefix.length), ClientSideIntlVariations[key]));
    });
    it('Gender config must match client-side equivalent ', () => {
      ...
      for (const k in IntlGenderVariations) {
        expect(ClientSideIntlVariations).toHaveProperty(prefix + k, IntlGenderVariations[k]);
      }

      getPropsByPrefix(ClientSideIntlVariations, prefix).forEach(key => expect(IntlGenderVariations).toHaveProperty(key.substr(prefix.length), ClientSideIntlVariations[key]));
    });
  });
});