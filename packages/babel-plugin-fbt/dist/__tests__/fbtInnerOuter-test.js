...
function testChildToParentRelationships(testData, expected) {
  ...
  try {
    assert.equal(JSON.stringify(expected, null, ' '), JSON.stringify(fbt.getChildToParentRelationships(), null, ' ')); // eslint-disable-next-line fb-www/no-unused-catch-bindings
  } catch (e) {
...
const testData = [{
  'should find the parent for a simple level': {
    input: withFbtRequireStatement(`<fbt desc="d">
  ...
  'should find the parents for a multi-nested level': {
    input: withFbtRequireStatement(`<fbt desc="phrase 0">
            <div>
