...
describe('Test declarative (jsx) <fbs> syntax translation', () => {
  it('should convert a simple string', () => {
    expect(snapshotTransform(withFbsRequireStatement(`const fbsElem = <fbs desc='str_description'>a simple string</fbs>;`))).toMatchSnapshot();
  });
  it('should convert a string with a parameter', () => {
    expect(snapshotTransform(withFbsRequireStatement(`
...
describe('Test functional fbs() syntax translation', () => {
  it('should convert a simple string', () => {
    expect(snapshotTransform(withFbsRequireStatement(`const fbsCall = fbs('a simple string', 'str_description');`))).toMatchSnapshot();
  });
  ...
  it('should convert a common string', () => {
    expect(snapshotTransform(withFbsRequireStatement(`const fbsCommonCall = fbs.c('Post');`))).toMatchSnapshot();
  ...
  it('should throw when using fbs() and the fbs variable is not bound', () => {
    expect(() => snapshotTransform(`const fbsCall = fbs(
          'basic',
          'str_description'
        );`)).toThrow(`fbs is not bound. Did you forget to require('fbs')?`);
  });
  it('should throw when using <fbs> and the fbs variable is not bound', () => {
    expect(() => snapshotTransform(`const fbsCall = <fbs desc="str_description">basic</fbs>;`)).toThrow(`fbs is not bound. Did you forget to require('fbs')?`);
  });
});