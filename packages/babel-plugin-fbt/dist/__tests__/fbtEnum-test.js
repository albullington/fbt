...
function runTest(data) {
  expect(snapshotTransform(data.input, {
...
describe('Test Fbt Enum', () => {
  ...
  it('should handle jsx enums (with references)', () => {
    runTest({
      input: withFbtRequireStatement(`let aEnum = require('Test$FbtEnum');
        var x = (
          <fbt desc="enums!">
            Click to see
  ...
  it('should throw when enum values are not strings', () => {
    expect(() => snapshotTransform(withFbtRequireStatement(`let aEnum = require('Test$FbtEnum');
  ...
  describe('when used with dynamic enum keys', () => {
    it('should throw the enum key is a variable (BabelNodeIdentifier)', () => {
      expect(() => snapshotTransform(withFbtRequireStatement(`const foo = 'anything';
    ...
    it('should throw the enum key is a variable (MemberExpression)', () => {
      expect(() => snapshotTransform(withFbtRequireStatement(`const foo = {bar: 'anything'};
            <fbt desc="try fbt:enum with a dynamic enum key">
              <fbt:enum
                enum-range={{
                  [foo.bar]: 'baz',
  ...
  it('should throw on multiple import types', () => {
    expect(() => snapshotTransform(withFbtRequireStatement(`import aEnum, * as bEnum from 'Test$FbtEnum';
          var x = fbt('Click to see ' + fbt.enum(id, aEnum), 'enums!');`))).toThrowError('Fbt Enum `aEnum` not registered');
  });
  it('should throw on destructured imports', () => {
    expect(() => snapshotTransform(withFbtRequireStatement(`import {aEnum} from 'Test$FbtEnum';
          var x = fbt('Click to see ' + fbt.enum(id, aEnum), 'enums!');`))).toThrowError('Fbt Enum `aEnum` not registered');
  });
});