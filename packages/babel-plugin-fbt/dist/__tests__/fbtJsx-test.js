...
const testData = {
  'should convert simple strings': {
    input: withFbtRequireStatement(`var x = <fbt desc="It's simple">A simple string</fbt>;`)
  },
  'should filter comment and empty expressions from children': {
    input: withFbtRequireStatement(`var x = <fbt desc="It's simple">
        {
        }A sim{/*
          ignore
          me
  ...
  'should strip out newlines': {
    input: withFbtRequireStatement(`var x =
  ...
  'Enable explicit whitespace': {
    input: withFbtRequireStatement(`var x = <fbt desc="squelched">
  ...
  'should handle params': {
    input: withFbtRequireStatement(`var x = <fbt desc="a message!">
  ...
  'should handle concatenated descriptions': {
    input: withFbtRequireStatement(`<fbt desc={"A very long description " + "that we will concatenate " +
        "a few times"}
  ...
  'should handle template descriptions': {
    input: withFbtRequireStatement(`<fbt desc={\`A very long description
        that will be a
  ...
  'should be able to nest within React nodes': {
    input: withFbtRequireStatement(`var x = <div>
        <fbt desc="nested!">
          A nested string
        </fbt>
      </div>;`)
  ...
  'should be able to house arbitrary markup within fbt:param nodes': {
    input: withFbtRequireStatement(`<div>
        <fbt desc="...">
  ...
  'should handle enums (with array values)': {
    input: withFbtRequireStatement(`var x = <fbt desc="enums!">
        Click to see
        <fbt:enum
          enum-range={[
            "groups",
  ...
  'should handle variations': {
    input: withFbtRequireStatement(`var x = <fbt desc="variations!">
        Click to see
  ...
  'should correctly destruct expression values in options': {
    input: withFbtRequireStatement(`<fbt desc="d">str
  ...
  'should treat multiline descs as a single line': {
    input: withFbtRequireStatement(`<fbt desc="hi how are you today im doing well i guess
  ...
  'should not insert extra space': {
    input: withFbtRequireStatement(`<fbt desc="Greating in i18n demo">
  ...
  'should handle single expression with concentated strings': {
    input: withFbtRequireStatement(`<fbt desc="foo">
        {"foo" + "bar"}
      </fbt>`)
  },
  'should throw on invalid attributes in fbt:param': {
    input: withFbtRequireStatement(`<fbt desc="some-desc">
  ...
  'should ignore __private attributes': {
    input: withFbtRequireStatement(`<fbt __self="fbt" desc="some-desc">
  ...
  'should support html escapes': {
    input: withFbtRequireStatement(`<fbt desc="foo &quot;bar&quot;">&times;</fbt>`)
  },
  'should support unicode characters': {
    input: withFbtRequireStatement(`// A backslash \\ in comments
  ...
  'should handle object pronoun': {
    input: withFbtRequireStatement(`<fbt desc={"d"} project={"p"}>
  ...
  'should handle subject+reflexive pronouns': {
    input: // She wished herself a happy birthday.
    withFbtRequireStatement(`<fbt desc={"d"} project={"p"}>
...
describe('Test declarative (jsx) fbt syntax translation', () => TestUtil.testSection(testData, snapshotTransformKeepJsx, {
  matchSnapshot: true
}));
describe('Test fbt transforms without the jsx transform', () => {
  it('not nested', () => {
    expect(snapshotTransformKeepJsx(`
  ...
  it(`[legacy buggy behavior] <fbt:pronoun> should insert a space character between two fbt constructs that don't neighbor raw text`, () => expect(snapshotTransformKeepJsx(`
        const fbt = require("fbt");
        <fbt desc="">
          You can add
