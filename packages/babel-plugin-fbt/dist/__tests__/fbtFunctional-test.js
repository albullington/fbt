...
const {
  TestUtil
} = require('fb-babel-plugin-utils'); // Given a test config's "filter" status, decides whether we should run it with
...
const generalTestData = {
  'should convert simple strings': {
    input: withFbtRequireStatement(`var x = fbt('A simple string', "It's simple");`),
    inputWithArraySyntax: withFbtRequireStatement(`var x = fbt(
        [
          'A simple string',
        ],
        "It's simple",
      );`),
    output: withFbtRequireStatement(`var x = fbt._(
        ${payload({
      jsfbt: {
        t: {
          desc: "It's simple",
  ...
  'should respect the doNotExtract option': {
    input: withFbtRequireStatement(`var x = fbt('A doNotExtract string', "should not be extracted", {doNotExtract: true});`),
    inputWithArraySyntax: withFbtRequireStatement(`var x = fbt(
        [
  ...
  'should allow description concatenation': {
    input: withFbtRequireStatement(`var x = fbt(
        'A short string',
  ...
  'should maintain newlines within arguments': {
    input: withFbtRequireStatement(`var z = fbt(
        'a' +
        ' b ' +
    ...
    inputWithArraySyntax: withFbtRequireStatement(`var z = fbt(
        [
          'a',
          ' b ',
          fbt.param('name1', val1),
    ...
    output: withFbtRequireStatement(`var z = fbt._(
        ${payload({
      jsfbt: {
        t: {
          desc: 'a',
  ...
  'should be able to nest within React nodes': {
    input: withFbtRequireStatement(`var React = require('react');
      var x = <div>{fbt('A nested string', 'nested!')}</div>;`),
    inputWithArraySyntax: withFbtRequireStatement(`var React = require('react');
      var x = <div>{fbt(['A nested string'], 'nested!')}</div>;`),
    output: withFbtRequireStatement(`var React = require('react');
  ...
  'should accept well-formed options': {
    input: withFbtRequireStatement(`fbt('A string that moved files', 'options!', {
        author: 'jwatson',
        project: 'Super Secret',
      });`),
    inputWithArraySyntax: withFbtRequireStatement(`fbt(['A string that moved files'], 'options!', {
        author: 'jwatson',
        project: 'Super Secret',
      });`),
    output: withFbtRequireStatement(`fbt._(
        ${payload({
      jsfbt: {
        t: {
          desc: 'options!',
  ...
  'should handle enums (with enum range as variable)': {
    ...
    input: withFbtRequireStatement(`var aEnum = require('Test$FbtEnum');
      var x = fbt(
        'Click to see ' + fbt.enum('id1', aEnum),
        'enum as an array',
      );`),
    inputWithArraySyntax: withFbtRequireStatement(`var aEnum = require('Test$FbtEnum');
      var x = fbt(
        [
          'Click to see ',
          fbt.enum('id1', aEnum),
    ...
    output: withFbtRequireStatement(`var aEnum = require('Test$FbtEnum');
      var x = fbt._(
        ${payload({
      jsfbt: {
        t: {
  ...
  'should handle variations + same param': {
    input: withFbtRequireStatement(`var val = 42;
      fbt(
        'You have ' +
    ...
    inputWithArraySyntax: withFbtRequireStatement(`var val = 42;
      fbt(
        [
          'You have ',
          fbt.param('count', val, {number: true}),
    ...
    output: withFbtRequireStatement(`var val = 42;
      fbt._(
        ${payload({
      jsfbt: {
        t: {
  ...
  'should get project from docblock': {
    input: `/** @fbt {"project": "dev"}*/
      ${withFbtRequireStatement(`var x = fbt('Also simple string', "It's simple");`)}`,
    inputWithArraySyntax: `/** @fbt {"project": "dev"}*/
      ${withFbtRequireStatement(`var x = fbt(['Also simple string'], "It's simple");`)}`,
    output: `/** @fbt {"project": "dev"}*/
      ${withFbtRequireStatement(`var x = fbt._(
          ${payload({
      jsfbt: {
        t: {
          desc: "It's simple",
  ...
  'should handler wrapping parens': {
    input: withFbtRequireStatement(`var x = fbt('foo' + 'bar' + 'baz' + 'qux', 'desc');
  ...
  'should handle subject and reflexive pronouns': {
    input: // I.e. He wished himself a happy birthday.
    withFbtRequireStatement(`var x = fbt(
    ...
    inputWithArraySyntax: // I.e. He wished himself a happy birthday.
    withFbtRequireStatement(`var x = fbt(
          [
            fbt.pronoun('subject', gender, {capitalize: true, human: true}),
            ' wished ',
  ...
  'should handle possessive pronoun': {
    input: // I.e. It is her birthday.
    withFbtRequireStatement(`var x = fbt(
          'It is ' + fbt.pronoun('possessive', gender) + ' birthday.',
          'possessive pronoun',
        );`),
    inputWithArraySyntax: // I.e. It is her birthday.
    withFbtRequireStatement(`var x = fbt(
          [
            'It is ',
  ...
  'should throw on pronoun usage not StringLiteral': {
    input: // Note use of variable for pronoun usage.
    withFbtRequireStatement(`var u = 'possessive';
        var x = fbt(
          'It is ' + fbt.pronoun(u, gender) + ' birthday.',
          'throw not StringLiteral',
        );`),
    inputWithArraySyntax: // Note use of variable for pronoun usage.
    withFbtRequireStatement(`var u = 'possessive';
        var x = fbt(
          [
            'It is ',
    ...
    throws: '`usage`, the first argument of fbt.pronoun() must be a `StringLiteral` but we got `Identifier`'
  },
  'should throw on pronoun usage invalid': {
    input: // Note 'POSSESSION' instead of 'possessive'.
    withFbtRequireStatement(`var x = fbt(
          'It is ' + fbt.pronoun('POSSESSION', gender) + ' birthday.',
          'throw because of unknown pronoun type',
        );`),
    inputWithArraySyntax: // Note 'POSSESSION' instead of 'possessive'.
    withFbtRequireStatement(`var x = fbt(
          [
            'It is ',
    ...
    throws: `\`usage\`, the first argument of fbt.pronoun() - ` + `Expected value to be one of [object, possessive, reflexive, subject] ` + `but we got 'POSSESSION' (string) instead`
  },
  'should throw when concatenating an fbt construct to a string while using the array argument syntax': {
    ...
    throws: 'fbt(array) only supports items that are string literals, ' + 'template literals without any expressions, or fbt constructs'
  },
  'should throw for string with a nested JSX fragment and subject gender contains function calls': {
    ...
    throws: `Expected string variation runtime argument "subject" ` + `to not be a function call or class instantiation expression.`
  },
  'should throw for string with a nested JSX fragment and string variation arguments that have nested class instantiation.': {
    ...
    throws: `Expected string variation runtime argument "count" ` + `to not contain a function call or class instantiation expression.`
  },
  'should throw for string with a nested JSX fragment and string variation arguments that have nested function calls (fbt:plural).': {
    ...
    throws: `Expected string variation runtime argument "count" ` + `to not be a function call or class instantiation expression.`
  },
  'should throw for string with a nested JSX fragment and string variation arguments that have nested function calls (fbt:enum).': {
    ...
    throws: `Expected string variation runtime argument "value" ` + `to not be a function call or class instantiation expression.`
  },
  'should throw for string with a nested JSX fragment and string variation arguments that have nested function calls (fbt:name).': {
    ...
    throws: `Expected string variation runtime argument "gender" ` + `to not be a function call or class instantiation expression.`
  },
  'should throw for string with a nested JSX fragment and string variation arguments that have nested function calls (fbt:param with number).': {
    ...
    throws: `Expected string variation runtime argument "number" ` + `to not be a function call or class instantiation expression.`
  },
  ...
  describe('Meta-data collection', () => {
    ...
    function withThrowExpectation(throwExpectation, callback) {
      return () => {
        ...
        } else if (typeof throwExpectation === 'string' || throwExpectation instanceof RegExp) {
    ...
    function testFbtMetaData(title, singleTestData, options) {
      $it(singleTestData)(`for scenario "${title}"`, withThrowExpectation(singleTestData.throws, () => {
