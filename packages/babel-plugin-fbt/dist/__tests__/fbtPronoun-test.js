...
describe('fbt pronoun support', () => {
  it('"capitalize" option accepts boolean literal true', () => {
    runTest({
      input: // eslint-disable-next-line fb-www/gender-neutral-language
  ...
  it('Should throw when using non-Boolean option value', () => {
    ...
    const input = withFbtRequireStatement(`var x = fbt(
        'Today is ' +
          fbt.pronoun('possessive', gender, {human: 'true'}) +
          ' a happy birthday.',
        'Expect error exception',
      );`);
    expect(() => transform(input)).toThrowError("Expected boolean value instead of 'true' (string)");
  });
  ...
  it('Should throw when using unknown "usage" value', () => {
    ...
    expect(() => transform(input)).toThrowError('`usage`, the first argument of fbt.pronoun() - Expected value to be ' + "one of [object, possessive, reflexive, subject] but we got 'possession' (string) instead");
  });
  it('Should elide false "human" option from fbt.pronoun()', () => {
    runTest({
      input: // I.e. Wish them a happy birthday.
      withFbtRequireStatement(`var x = fbt(
            'Wish ' +
...
describe('fbt pronoun support (react native)', () => {
  ...
  it('Should throw when using non-Boolean option value', () => {
    ...
    })).toThrowError("Expected boolean value instead of 'true' (string)");
  });
  it('Should throw when using non-Boolean option value in a template', () => {
    // Note: Using StringLiteral '"true"' instead of BooleanLiteral 'true'.
    const input = withFbtRequireStatement(`var x = fbt(
  ...
  it('Should throw when using unknown "usage" value', () => {
    ...
    expect(() => transform(input, {
      reactNativeMode: true
    })).toThrowError('`usage`, the first argument of fbt.pronoun() - Expected value to be ' + "one of [object, possessive, reflexive, subject] but we got 'possession' (string) instead");
  });
  it('Should elide false "human" option from fbt.pronoun()', () => {
    runTestForReactNative({
      input: // I.e. Wish them a happy birthday.
