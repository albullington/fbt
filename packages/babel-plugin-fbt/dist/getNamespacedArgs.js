...
const getNamespacedArgs = function (moduleName) {
  return {
    ...
    param(node) {
      ...
      const options = getOptionsFromAttributes(t, attributes, ValidParamOptions, RequiredParamOptions);
      let paramChildren = filterEmptyNodes(node.children).filter(function (child) {
      ...
      if (paramChildren.length !== 1) {
        throw errorAt(node, `${moduleName}:param expects an {expression} or JSX element, and only one`);
      }
      ...
      const paramArgs = [nameAttrValue, paramChildren[0].expression || paramChildren[0]];
    ...
    plural(node) {
      const attributes = node.openingElement.attributes;
      const options = getOptionsFromAttributes(t, attributes, PluralOptions, PluralRequiredAttributes);
      ...
      if (pluralChildren.length !== 1) {
        throw errorAt(node, `${moduleName}:plural expects text or an expression, and only one`);
      }

      const singularNode = pluralChildren[0];
      const singularText = expandStringConcat(moduleName, singularNode.expression || singularNode);
      const singularArg = stringLiteral(normalizeSpaces(singularText.value).trimRight());
    ...
    pronoun(node) {
      if (!node.openingElement.selfClosing) {
        throw errorAt(node, `${moduleName}:pronoun must be a self-closing element`);
      }
      ...
      if (typeAttr.type !== 'StringLiteral') {
        throw errorAt(node, `${moduleName}:pronoun attribute "type" must have StringLiteral content`);
      }

      if (!ValidPronounUsages.hasOwnProperty(typeAttr.value)) {
        throw errorAt(node, `${moduleName}:pronoun attribute "type" must be one of [` + Object.keys(ValidPronounUsages) + ']');
      }
      ...
      const options = getOptionsFromAttributes(t, attributes, ValidPronounOptions, PronounRequiredAttributes);
    ...
    name(node) {
      ...
      const genderAttribute = getAttributeByNameOrThrow(attributes, 'gender').value;
      const children = filterEmptyNodes(node.children);
      ...
      if (nameChildren.length !== 1) {
        throw errorAt(node, `${moduleName}:name expects text or an expression, and only one`);
      }
    ...
    sameParam(node) {
      if (!node.openingElement.selfClosing) {
        throw errorAt(node, `Expected ${moduleName}:same-param to be selfClosing.`);
      }

      const nameAttr = getAttributeByNameOrThrow(node.openingElement.attributes, 'name');
    ...
    enum(node) {
      ...
      const rangeAttr = getAttributeByNameOrThrow(node.openingElement.attributes, 'enum-range');
      ...
      const valueAttr = getAttributeByNameOrThrow(node.openingElement.attributes, 'value');
      ...
      throw errorAt(node, `Expected value attribute of <${moduleName}:enum> to be an expression ` + `but got ${valueAttr.value.type}`);
    }
