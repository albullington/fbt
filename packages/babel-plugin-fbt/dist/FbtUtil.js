...
/* eslint-disable fb-www/no-new-error */
// disable www-only linter
'use strict';
...
function normalizeSpaces(value, // TODO(T56277500) set better types for Fbt options object to use `preserveWhitespace?: ?boolean`
options) {
...
function validateNamespacedFbtElement(moduleName, node) {
  let valid = false;
  let handlerName; // Actual namespaced version, e.g. <fbt:param>

  if (isJSXNamespacedName(node)) {
    handlerName = node.name.name;
    valid = isJSXIdentifier(node.namespace) && node.namespace.name === moduleName && (handlerName === 'enum' || handlerName === 'param' || handlerName === 'plural' || handlerName === 'pronoun' || handlerName === 'name' || handlerName === 'same-param'); // React's version, e.g. <FbtParam>, or <FbtEnum>
  ...
  !(handlerName != null) ? process.env.NODE_ENV !== "production" ? invariant(false, 'handlerName must not be null') : invariant(false) : void 0;
  return handlerName;
}

function isBabelNodeCallExpressionArg(value) {
  return isExpression(value) || isSpreadElement(value) || isJSXNamespacedName(value) || isArgumentPlaceholder(value);
}
...
function setUniqueToken(node, moduleName, name, paramSet) {
  ...
  if (cachedNode && cachedNode != node) {
    throw errorAt(node, `There's already a token called "${name}" in this ${moduleName} call. ` + `Use ${moduleName}.sameParam if you want to reuse the same token name or ` + `give this token a different name`);
  }
...
function checkOption(option, validOptions, value) {
  ...
  if (!validOptions.hasOwnProperty(optionName) || validValues == null) {
    throw errorAt(isNode(value) ? value : null, `Invalid option "${optionName}". ` + `Only allowed: ${Object.keys(validOptions).join(', ')} `);
    ...
    if (typeof value === 'string' || typeof value === 'boolean') {
      ...
      throw errorAt(isNode(value) ? value : null, `Option "${optionName}" has an invalid value. ` + `Expected a string literal but value is \`${varDump(value)}\` (${typeof value})`);
    }

    if (!validValues[valueStr]) {
      throw errorAt(isNode(value) ? value : null, `Option "${optionName}" has an invalid value: "${valueStr}". ` + `Only allowed: ${Object.keys(validValues).join(', ')}`);
    }
  }
...
function getOptionsFromAttributes(t, attributesNode, validOptions, ignoredAttrs) {
  ...
  attributesNode.forEach(function (node) {
    const option = node.name.name; // Required attributes are passed as a separate argument in the fbt(...)
 ...
 */
function errorAt(astNode, msgOrError = '', options = {}) {
  let error;

  if (typeof msgOrError === 'string') {
    const newError = new Error(createErrorMessageAtNode(astNode, msgOrError, options)); // $FlowExpectedError[incompatible-exact] Allow setting some internal props on Error object

    error = newError; // $FlowExpectedError[prop-missing] Allow setting some internal props on Error object
    ...
    if (error._hasBabelNodeLocation !== true) {
      error.message = createErrorMessageAtNode(astNode, error.message, options); // $FlowExpectedError[prop-missing] Allow setting some internal props on Error object
      // $FlowExpectedError[incompatible-use]
...
function createErrorMessageAtNode(astNode, msg = '', options = {}) {
  const location = astNode && astNode.loc;
  const optionalMessage = options.suggestOSSWebsite ? 'See the docs at https://facebook.github.io/fbt/ for more info.' : null;
  return (location != null ? `Line ${location.start.line} Column ${location.start.column + 1}: ` : '') + msg + (optionalMessage ? `\n${optionalMessage}` : '') + (astNode != null ? `\n---\n${generateFormattedCodeFromAST(astNode)}\n---` : '');
...
function collectOptions(moduleName, options, validOptions) {
  ...
  options.properties.forEach(option => {
    ...
    if (isArrowFunctionExpression(option.value)) {
      throw errorAt(option, `${moduleName}(...) does not allow an arrow function as an option value`);
    }

    const value = option.value.expression || option.value; // Append only default valid options excluding "extraOptions",
...
function collectOptionsFromFbtConstruct(moduleName, callsiteNode, validOptions, booleanOptions = null) {
  let optionsNode;
  let options = {};
...
function getOptionsNodeFromCallExpression(moduleName, node) {
  ...
  if (!isObjectExpression(optionsNode)) {
    throw errorAt(optionsNode, `${moduleName}(...) expects options as an ObjectExpression as its 3rd argument`);
  }
...
function expandStringConcat(moduleName, node) {
  if (isBinaryExpression(node)) {
    if (node.operator !== '+') {
      throw errorAt(node, `Expected concatenation operator (+) but got ${node.operator}`);
    }
    ...
    for (const elem of node.quasis) {
      ...
      if (index < expressions.length) {
        const expr = expressions[index++]; // fbt.param expressions are already transformed to StringLiteral

        if (!isStringLiteral(expr)) {
          throw errorAt(node, `${moduleName} template placeholders only accept params wrapped in ` + `${moduleName}.param. Expected StringLiteral got ${expr.type}`);
        }
  ...
  throw errorAt(node, `${moduleName} only accepts plain strings with params wrapped in ${moduleName}.param(...). ` + `See the docs at https://facebook.github.io/fbt/ for more info. ` + `Expected StringLiteral, TemplateLiteral, or concatenation; ` + // $FlowExpectedError This BabelNode is unsupported so it may not even have a type property
  `got "${node.type}"`);
}

function expandStringArray(moduleName, node) {
  return stringLiteral(nullthrows(node.elements).map(element => expandStringConcat(moduleName, nullthrows(element)).value).join(''));
...
function getOptionBooleanValue(options, name, node) {
  ...
  if (value.expression) {
    ...
    throw errorAt(node, `Value for option "${name}" must be Boolean literal 'true' or 'false'.`);
  }
}
/**
...
function getAttributeByNameOrThrow(attributes, name, node = null) {
...
function extractEnumRange(node) {
  return node.properties.reduce((acc, prop) => {
    if (!isObjectProperty(prop)) {
      throw new Error(`fbt enum range properties must be ObjectProperty, got ${prop.type}`);
    }

    if (!isStringLiteral(prop.value)) {
      throw new Error(`fbt enum range property values must be StringLiteral, got ${prop.value.type}`);
    }
...
function objMap(object, fn) {
  ...
  if (!(object instanceof Object) || // $FlowExpectedError "prototype" property might not exist
  ...
  for (const k in object) {
    toMap[k] = fn(object[// `k` is normally a `string` but it is inferred to be TKey
    k], k);
  }
...
function getRawSource(src, node) {
  return src.substring( // $FlowFixMe node.start might be undefined
  node.start, // $FlowFixMe node.end might be undefined
  node.end);
}
/**
...
function convertTemplateLiteralToArrayElements(moduleName, node) {
  ...
  const nodes = [];
  let index = 0; // quasis items are the text literal portion of the template literal
  ...
  for (const item of quasis) {
    ...
    if (index < expressions.length) {
      ...
      if (expression.type === 'StringLiteral' || expression.type === 'CallExpression' || expression.type === 'JSXElement') {
        ...
        throw errorAt(expression, `Unexpected node type: ${expression.type}. ${moduleName}() only supports ` + `the following syntax within template literals:` + `string literal, a construct like ${moduleName}.param() or a JSX element.`);
      }
    }
  }
...
function getBinaryExpressionOperands(moduleName, node) {
  switch (node.type) {
    case 'BinaryExpression':
      ...
      return [...getBinaryExpressionOperands(moduleName, node.left), ...getBinaryExpressionOperands(moduleName, node.right)];
    ...
    default:
      throw errorAt(node, `Unexpected node type: ${node.type}. ` + `The ${moduleName}() string concatenation pattern only supports ` + ` string literals or constructs like ${moduleName}.param().`);
  }
}
...
function convertToStringArrayNodeIfNeeded(moduleName, node) {
  ...
  switch (node.type) {
    ...
    default:
      throw errorAt(node, `Unexpected node type: ${node.type}. ` + `${moduleName}()'s first argument should be a string literal, ` + `a construct like ${moduleName}.param() or an array of those.`);
  ...
  return arrayExpression(initialElements.reduce((elements, element) => {
    ...
    if (didStartWithArray && (element.type === 'BinaryExpression' || element.type === 'TemplateLiteral' && element.expressions.length)) {
      throw errorAt(element, `${moduleName}(array) only supports items that are string literals, ` + `template literals without any expressions, or fbt constructs`);
    }
...
function compactBabelNodeProps(object, serializeSourceCode = true) {
...
function varDump(value, depth = 1) {
...
function enforceString(value, valueDesc) {
  !(typeof value === 'string') ? process.env.NODE_ENV !== "production" ? invariant(false, '%sExpected string value instead of %s (%s)', valueDesc ? valueDesc + ' - ' : '', varDump(value), typeof value) : invariant(false) : void 0;
  return value;
}

function enforceBoolean(value, valueDesc) {
  !(typeof value === 'boolean') ? process.env.NODE_ENV !== "production" ? invariant(false, '%sExpected boolean value instead of %s (%s)', valueDesc ? valueDesc + ' - ' : '', varDump(value), typeof value) : invariant(false) : void 0;
  return value;
}

function enforceBabelNode(value, valueDesc) {
  !isNode(value) ? process.env.NODE_ENV !== "production" ? invariant(false, '%sExpected BabelNode value instead of %s (%s)', valueDesc ? valueDesc + ' - ' : '', varDump(value), typeof value) : invariant(false) : void 0;
  return value;
}

function enforceBabelNodeExpression(value, valueDesc) {
  !isExpression(value) ? process.env.NODE_ENV !== "production" ? invariant(false, '%sExpected BabelNodeExpression value instead of %s (%s)', valueDesc ? valueDesc + ' - ' : '', varDump(value), typeof value) : invariant(false) : void 0;
  return value;
}

function enforceBabelNodeCallExpressionArg(value, valueDesc) {
  !isBabelNodeCallExpressionArg(value) ? process.env.NODE_ENV !== "production" ? invariant(false, '%sExpected BabelNodeCallExpressionArg value instead of %s (%s)', valueDesc ? valueDesc + ' - ' : '', varDump(value), typeof value) : invariant(false) : void 0;
  return value;
}

function enforceStringEnum(value, keys, valueDesc) {
  !(typeof value === 'string' && keys.hasOwnProperty(value)) ? process.env.NODE_ENV !== "production" ? invariant(false, '%sExpected value to be one of [%s] but we got %s (%s) instead', valueDesc ? valueDesc + ' - ' : '', Object.keys(keys).join(', '), varDump(value), typeof value) : invariant(false) : void 0; // $FlowFixMe[incompatible-return] Value has been refined with the above invariant check
...
enforceBabelNodeExpression.orNull = nullableTypeCheckerFactory(enforceBabelNodeExpression);
enforceBabelNodeCallExpressionArg.orNull = nullableTypeCheckerFactory(enforceBabelNodeCallExpressionArg);
...
function createFbtRuntimeArgCallExpression(fbtNode, args, overrideMethodName) {
  return callExpression(memberExpression(identifier(fbtNode.moduleName), identifier('_' + (overrideMethodName || nullthrows( // $FlowExpectedError[prop-missing] using nullthrows() to detect if it's undefined
  fbtNode.constructor.type)))), args);
}
/**
...
function replaceClearTokensWithTokenAliases(textOrTranslation, tokenAliases) {
  ...
  return Object.keys(tokenAliases).reduce((mangledText, clearToken) => {
    const clearTokenName = tokenNameToTextPattern(clearToken);
    const mangledTokenName = tokenNameToTextPattern(tokenAliases[clearToken]); // Since a string is not allowed to have implicit params with duplicated
